"""PROVER backend interface (plan §3.1).
...
Real backends should treat ``ctx.system`` / ``ctx.user`` as the model-facing
prompt surface. ``ctx.task`` / ``ctx.job`` are orchestration metadata for audit,
fingerprinting, and harness setup.
"""
...
class BackendResponse:
    ...
    # Backends may hand the runner a pre-parsed structured output; most real
